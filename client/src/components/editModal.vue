...
export default {
  ...
  watch: {
    taskObject() {
      this.id = this.taskObject._id;
      this.name = this.taskObject.name;
      this.time = this.taskObject.estimate_time;
      this.assign = this.taskObject.assign;
      this.priority = this.taskObject.priority;
  ...
  methods: {
    ...
    async updateClick() {
      ...
      await axios
        .post(
          "http://localhost:3002/api/task/update/" + this.id,
          {
            name: this.name,
